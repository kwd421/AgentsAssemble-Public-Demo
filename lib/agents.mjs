export const AGENTS = [
  { id: 'strategist', mention: 'ai1', name: '전략가', label: 'Strategy', initial: 'S', modelEnv: 'AI_MODEL_STRATEGIST', role: '목표 · 우선순위 · 구조', prompt: '질문의 핵심과 전제를 정리하고 먼저 직접 답하세요. 복잡한 계획에서는 우선순위를 제안하세요.' },
  { id: 'engineer', mention: 'ai2', name: '엔지니어', label: 'Engineering', initial: 'E', modelEnv: 'AI_MODEL_ENGINEER', role: '구현 · 근거 · 기술 리스크', prompt: '구현 질문에서는 기술·비용·실행 가능성을 평가하세요. 그 외 질문에서는 관련 근거와 논리를 점검하세요. 만화나 일상 질문에 소프트웨어 비유를 억지로 적용하지 마세요.' },
  { id: 'critic', mention: 'ai3', name: '비평가', label: 'Critical Review', initial: 'C', modelEnv: 'AI_MODEL_CRITIC', role: '허점 · 검증 · 실패 조건', prompt: '앞선 발언의 중요한 불확실성과 논리적 오류를 검토하세요. 무조건 반박하지 마세요. 확인할 근거가 없으면 새로운 정답이나 세부 수치를 만들어 반박하지 마세요.' },
];
...
function hasNameMention(content, name) {
  const value = String(name || '').trim();
  if (!value) return false;
  return new RegExp(`(?:^|\\s)@${escapeRegExp(value)}(?=$|\\s|[.,!?，。！？:;])`, 'iu').test(String(content));
}
export function selectAgents(content, targetAgentId, agents = AGENTS) {
  ...
  const tokens = [...text.matchAll(/(?:^|\s)@([\p{L}\p{N}_-]+)(?=$|\s|[.,!?，。！？:;])/gu)].map(m => m[1].toLowerCase());
  if (tokens.includes('all') || tokens.includes('전체')) return agents;
  const selected = agents.filter(a => tokens.includes(String(a.mention || '').toLowerCase()) || tokens.includes(a.id.toLowerCase()) || hasNameMention(text, a.name));
