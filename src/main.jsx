...
import { AtSign, Bot, ChevronDown, Hash, Home, RefreshCw, Search, Send, Square, Users } from 'lucide-react';
...
function App() {
  const { room, draft, setDraft, connection, error, setError, pending, submit, retry, cancel, reset } = useRoom();
  ...
  return <div className="dc-shell flex h-screen max-h-screen overflow-hidden text-text-primary">
    <nav className="dc-rail flex shrink-0 flex-col items-center gap-2 py-3" aria-label="방 목록">
      <button className="dc-rail-home" data-active="true" onClick={reset} disabled={busy} title="새 임시 체험방" aria-label="새 체험방"><Home size={22} /></button>
    ...
    <aside className="dc-sidebar flex shrink-0 flex-col" aria-label="채널 목록">
      <header className="dc-sidebar-head shrink-0" data-tone="violet"><button className="dc-server-header-button" onClick={reset} disabled={busy} title="새 임시 체험방 시작"><span className="truncate">AgentsAssemble Demo</span><ChevronDown size={16} /></button>
