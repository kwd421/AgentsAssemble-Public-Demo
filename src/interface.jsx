...
function AgentBanner({ agent, name, index, compact = false }) {
  return <div className={`aa-profile-identity${compact ? ' compact' : ''}`}>
    ...
    <div className="aa-profile-title"><AgentAvatar agent={agent} large /><span className="aa-role-tag">{agent.role || '역할 미설정'}</span><h3>{name || agent.name}</h3><span className="aa-profile-handle">@{agent.mention || `ai${(index ?? 0) + 1}`}</span></div>
  </div>;
}

export function PromptTip({ compact = false }) {
  return <details className={`aa-writing-note${compact ? ' compact' : ''}`}>
    <summary><BookOpen size={16} aria-hidden="true" /><span>지시문 작성 요령</span><ChevronRight size={15} className="aa-note-chevron" aria-hidden="true" /></summary>
    <div className="aa-writing-body"><p>역할, 판단 기준, 답변 형식을 구체적으로 적습니다. “잘 답하기”보다는 어떤 답변이 필요한지 설명합니다.</p><blockquote>구현 가능성을 먼저 검토하세요. 결론, 이유, 리스크 순으로 짧게 답하고, 확인할 수 없는 수치는 추측하지 마세요.</blockquote></div>
  </details>;
}
    ...
    <div className="aa-dialog-inner">
      ...
      {page === 0 ? <>
        <div className="aa-dialog-scroll aa-welcome" ref={scrollArea}>
          ...
          <div className="aa-welcome-roster"><div className="aa-roster-heading">대화에 참여하는 에이전트<span>3</span></div>{['strategist', 'engineer', 'critic'].map((id, index) => { const agent = agents.find(a => a.id === id) || { id, mention: `ai${index + 1}`, name: '연결 중…', initial: id[0].toUpperCase(), role: '에이전트 정보를 불러오는 중' }; return <div className="aa-welcome-member" data-agent={id} key={id}><AgentAvatar agent={agent} /><div><strong>{agent.name}</strong><p>@{agent.mention || `ai${index + 1}`} · {agent.role || '역할 미설정'}</p></div><span className="aa-member-order">0{index + 1}</span></div>; })}<p className="aa-roster-note">각 에이전트는 앞선 답변을 읽고<br />자신의 역할에 따라 이어서 답합니다.</p></div>
        ...
        <div className="aa-dialog-scroll" ref={scrollArea}>
          ...
          <div className="aa-setup-notes"><p><Pencil size={16} />역할, 판단 기준, 답변 형식을 정하면 지시가 더 명확해집니다.</p><PromptTip /></div>
